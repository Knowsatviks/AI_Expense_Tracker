...
export const analyzeTransactionList = async ({
  ...
  const prompt = `Analyze these ${transactions.length} transactions and provide a concise, helpful spending insight. Focus on patterns, notable observations.
...
export const analyzeBudgetList = async ({ budgets, currency = "USD" }) => {
  ...
  const prompt = `You're a personal finance assistant. Analyze each budget below and provide a concise, personalized one-sentence assessment based on the percentage spent, including encouragement when spending is under control and practical advice when spending is approaching or exceeding the budget.
